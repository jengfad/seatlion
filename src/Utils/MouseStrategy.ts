...
export class Shape extends Strategy {
  ...
  public handleMouseDown = (e: MouseEvent<any>) => {
    ...
    const { getElementProps, setFocusedComponentId, setWhiteboardCursor, setActiveSelection, activeSelection } =
      useStore.getState();
    ...
    if (e.ctrlKey) {
      setActiveSelection([...activeSelection, this.id]);
    }
  ...
  private moveShapes = (cursor: { x: number; y: number }) => {
    const { setElementProps, activeSelection, board } = useStore.getState();
    const deltaX = cursor.x - this.startPoint.x;
    const deltaY = cursor.y - this.startPoint.y;

    activeSelection.forEach(id => {
      const shape = board.find(shape => shape.id === id);
      if (shape) {
        setElementProps(id, {
  ...
  public handleMouseMove = (e: MouseEvent<any>): void => {
    const { setElementProps, setShapeEditor, activeSelection } = useStore.getState();
